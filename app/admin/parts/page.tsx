...
async function fetchParts(params: {
  ...
  const res = await fetch(`/api/admin/parts-datatable?${sp.toString()}`, {
    cache: "no-store",
      ...
      <div className="border-b bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-gray-900">Parts Assistance</h1>
        ...
        <div className="rounded-2xl border bg-white shadow-sm">
          <div className="flex flex-col gap-2 border-b p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-gray-600">
              {loading ? "Loading…" : `Showing ${data.items.length} of ${data.total} parts requests`}
            </div>
              ...
              <tbody>
                {loading ? (
                  ...
                  data.items.map((r, idx) => (
                    <tr key={`${r.orderNumber}-${idx}`} className="border-t hover:bg-gray-50">
