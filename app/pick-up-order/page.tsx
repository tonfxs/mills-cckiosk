...
//                       placeholder="e.g., E1234567, 100000123456, BAUMR-1234"
//                     />
...
import { useState, useEffect, useRef } from "react";
import { ChevronRight, Package, UserRoundPen, PackageCheck, IdCardIcon, ShieldAlert } from "lucide-react";
import SuccessScreen from "@/app/components/SuccessScreen";
import Link from "next/link";
import CarParkBayPopup from "../components/CarParkPopUp";
import OtherPaymentPopup from "@/app/components/OtherPaymentPopup";
...
interface FormData {
  ...
  carParkBay: string;
  confirmed: boolean;
  disclaimerAcknowledged: boolean; // ✅ NEW
}
...
export default function PickupKiosk() {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    carParkBay: "",
    confirmed: false,
    disclaimerAcknowledged: false, // ✅ NEW
  });
  ...
  const validateStep = (stepNumber: number): string[] => {
    ...
    switch (stepNumber) {
      ...
      // ✅ NEW Step 4: Disclaimer acknowledgement
      case 4:
        if (!formData.disclaimerAcknowledged) {
          stepErrors.push("You must acknowledge the Pickup Risk disclaimer to proceed");
        }
        break;
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.validId) allErrors.push("Please select a valid ID");
    if (!formData.paymentMethod) allErrors.push("Please select a payment method");
    if (!formData.disclaimerAcknowledged) allErrors.push("You must acknowledge the Pickup Risk disclaimer"); // ✅ NEW
    if (!formData.carParkBay.trim()) allErrors.push("Car park bay is required");
    if (!formData.confirmed) allErrors.push("You must confirm the data");
  ...
  // ✅ Steps are now 1–5 (disclaimer is Step 4, confirm is Step 5)
  const TOTAL_STEPS = 5;
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white flex flex-col">
      ...
      <div className="bg-white shadow-sm border-b border-gray-200 p-6">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          {[
            ...
            { num: 2, label: "Contact Info", icon: UserRoundPen },
            { num: 3, label: "Select ID", icon: IdCardIcon },
            { num: 4, label: "Disclaimer", icon: ShieldAlert }, // ✅ NEW step in progress bar
            { num: 5, label: "Confirm", icon: PackageCheck },
          ].map(({ num, label, icon: Icon }) => (
            <div key={num} className="flex flex-col items-center flex-1">
              <div
                className={`w-20 h-20 rounded-full flex items-center justify-center text-2xl font-bold mb-2 transition-all ${
                  step >= num
                    ? num === 4
                      ? "bg-red-500 text-white" //  Distinct color for disclaimer step
      ...
      <div className="flex-1 p-8">
        <div className="max-w-4xl mx-auto">
          ...
          {/* ✅ Step 4: Pickup Risk Disclaimer (NEW) */}
          {step === 4 && (
            <div className="space-y-6">
              <div className="bg-white rounded-3xl shadow-xl p-10 border-4 border-red-400">
                {/* Header */}
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h2 className="text-4xl font-bold text-gray-800">Disclaimer & Acknowledgement</h2>
                    <p className="text-2xl text-red-600 font-semibold mt-1">(Please read carefully before proceeding.)</p>
                  </div>
                </div>

                {/* Disclaimer box */}
                <div className="bg-red-50 border-4 border-red-400 rounded-2xl p-8 mb-8">
                  <h3 className="text-3xl font-bold text-red-800 mb-6">Acknowledgement</h3>
                  <p className="text-2xl text-gray-800 mb-6 font-semibold">
                    By proceeding with the collection, you agree to the following:
                  </p>

                  <ul className="space-y-6">
                    <li className="flex gap-4 items-start">
                      <span className="mt-1 flex-shrink-0 w-8 h-8 bg-red-500 text-white rounded-full flex items-center justify-center text-xl font-bold">1</span>
                      <p className="text-2xl text-gray-800 leading-relaxed">
                        Acceptance of the risk of vehicle damage and waiving rights to any claim against any party related to vehicle damage.
                      </p>
                    </li>
                    <li className="flex gap-4 items-start">
                      <span className="mt-1 flex-shrink-0 w-8 h-8 bg-red-500 text-white rounded-full flex items-center justify-center text-xl font-bold">2</span>
                      <p className="text-2xl text-gray-800 leading-relaxed">
                        Alternatively, you may instruct warehouse staff to leave the carton away from the vehicle to reduce risk.
                      </p>
                    </li>
                    <li className="flex gap-4 items-start">
                      <span className="mt-1 flex-shrink-0 w-8 h-8 bg-red-500 text-white rounded-full flex items-center justify-center text-xl font-bold">3</span>
                      <p className="text-2xl text-gray-800 leading-relaxed">
                        Please check all boxes before leaving the warehouse, as we do not take responsibility for missing or damaged cartons once they leave the premises.
                      </p>
                    </li>
                  </ul>
                </div>
                ...
                <label className="flex items-start gap-6 p-8 bg-red-50 border-4 border-red-500 rounded-2xl cursor-pointer hover:bg-red-100 transition-all">
                  <input
                    type="checkbox"
                    name="disclaimerAcknowledged"
                    checked={formData.disclaimerAcknowledged}
                    onChange={handleChange}
                    className="w-14 h-14 mt-1 flex-shrink-0 accent-red-500"
                  />
                  <span className="text-2xl font-bold text-gray-800">
                    I have read and understood the above disclaimer. I acknowledge and accept these terms before proceeding with my pickup.
                  </span>
                </label>

                {!formData.disclaimerAcknowledged && (
                  <div className="mt-6 p-6 bg-red-50 border-4 border-red-300 rounded-2xl text-center">
                    <p className="text-2xl font-bold text-red-600">
                      ⚠ You must tick the checkbox above to proceed to the next step.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-3xl shadow-xl p-10">
                ...
                <div className="space-y-4 mb-8 bg-gray-50 p-8 rounded-2xl">
                  ...
                  {/* ✅ Show disclaimer acknowledgement status in review */}
                  <div className="flex justify-between text-4xl">
                    <span className="font-semibold text-gray-600">Risk Disclaimer:</span>
                    <span className="font-bold text-green-600">Acknowledged</span>
                  </div>
                ...
                try {
                  const emailRes = await fetch("/api/send-email", {
                    ...
                    body: JSON.stringify({
                      ...
                      validId: formData.validId,
                      carParkBay: formData.carParkBay,
                      disclaimerAcknowledged: formData.disclaimerAcknowledged, // ✅ pass to email
                    }),
                  });
